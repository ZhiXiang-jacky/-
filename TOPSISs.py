...
class Topsis():
    ...
    def step_3(self):
        ...
        self.weighted_normalized = np.copy(self.normalized_decision)  #normalized_decision 就是 r矩陣
        for i in range(self.row_size):
            for j in range(self.column_size):
    ...
    def step_4(self):
        ...
        for i in range(self.column_size):
            if self.criteria[i]:
                # '''最好的ˊbest_alternatives要望大'''  #望大
                self.worst_alternatives[i] = min(self.weighted_normalized[:, i])  #在望小的屬性上達最小值
                self.best_alternatives[i] = max(self.weighted_normalized[:, i])   #理想解乃是望大的屬性上達最大值
            else:
                # '''最好的ˊbest_alternatives要望小'''  #望小
                self.worst_alternatives[i] = max(self.weighted_normalized[:, i])  #在望大的屬性上達最小值
                self.best_alternatives[i] = min(self.weighted_normalized[:, i])   #理想解在望小的屬性上達最大值
    ...
    def step_5(self):
        self.worst_distance = np.zeros(self.row_size)       #100個最不好
        self.best_distance = np.zeros(self.row_size)        #100個最好
        ...
        for i in range(self.row_size):
            for j in range(self.column_size):
                self.worst_distance_mat[i][j] = (self.weighted_normalized[i][j]-self.worst_alternatives[j])**2 # S-
                self.best_distance_mat[i][j] = (self.weighted_normalized[i][j]-self.best_alternatives[j])**2   # S*
...
# =============================================================================
             #calculate the similarity to the worst condition
             self.worst_similarity[i] = self.best_distance[i] / (self.worst_distance[i]+self.best_distance[i])
 
             # calculate the similarity to the best condition
             self.best_similarity[i] =  self.worst_distance[i] / (self.worst_distance[i]+self.best_distance[i])
    
    def rank_to_worst_similarity(self):
        return rankdata(self.worst_similarity, method="min").astype(int)    #method="min" 是遇到相同的就用小的當作一樣

    def rank_to_best_similarity(self):
        return rankdata(self.best_similarity, method='min').astype(int)     #method="min" 是遇到相同的就用小的當作一樣
    ...
    def calc(self):
        ...
        # rankdata([-1 * i for i in self.best_distance], method='min').astype(int)
        self.TOPSIS_result = pd.DataFrame([self.best_similarity,rankdata([1 * i for i in self.best_similarity], method='min').astype(int)]).T
        # self.TOPSIS_result = pd.DataFrame([self.best_similarity,rankdata(self.best_distance, method='min')]).T
        self.TOPSIS_result.columns = ['Score','Rank']
        self.TOPSIS_result.fillna(0) # 若結果一直收斂，計算的直將會變成 nan 因此計算不出距離需為 0 20210605  
        #columns = ['Score','Rank']
        self.TOPSIS_result['Rank'] = self.TOPSIS_result['Rank'].astype(int)
        return self.TOPSIS_result
